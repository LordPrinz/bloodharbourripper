...
public class BoneSkewerRawItem extends SwordItem {
    public BoneSkewerRawItem(Tier tier, int attackDamageModifier, float attackSpeedModifier, Properties properties) {
    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand hand) {
        ...
        if (BoneSkewerTracker.hasActiveSkewer(player)) {
            ...
            if (!level.isClientSide) {
                ...
                if (activeSkewer instanceof BoneSkewerRawEntity skewer) {
                    skewer.forceReturn();
        ...
        if (entity instanceof Player player) {
            ...
            if (i >= 10) {
                ...
                if (!level.isClientSide) {
                    ...
                    BoneSkewerRawEntity skewer = new BoneSkewerRawEntity(level, player, stack);
                    // Rzut jak trident - większa prędkość i odległość
                    skewer.shootFromRotation(player, player.getXRot(), player.getYRot(), 0.0F, 2.5F, 1.0F);
